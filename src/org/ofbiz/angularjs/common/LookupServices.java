...
public class LookupServices {
    ...
    public static Map<String, Object> performLookupJsonList(
        ...
        boolean searchDistinct = Boolean.valueOf((String) context
                .remove("searchDistinct"));
        String term = (String) context.remove("term");
        String searchValueFieldName = (String) context
                .remove("searchValueFieldName");
        String searchType = (String) context.remove("searchType");
        List<String> conditionFields = (List) context.remove("conditionFields");
        EntityCondition andCondition = (EntityCondition) context
                .remove("andCondition");
        ...
        if (UtilValidate.isNotEmpty(searchFields)
                && UtilValidate.isNotEmpty(fieldValue)) {
            ...
            for (String fieldName : searchFieldsList) {
                if ("EQUALS".equals(searchType)) {
                    orExprs.add(EntityCondition.makeCondition(EntityFieldValue
                            .makeFieldValue(searchFieldsList.get(0)),
                            EntityOperator.EQUALS, searchValue));
                } else {
                    orExprs.add(EntityCondition.makeCondition(EntityFunction
                            .UPPER(EntityFieldValue.makeFieldValue(fieldName)),
        ...
        if ((UtilValidate.isNotEmpty(orExprs)
                && UtilValidate.isNotEmpty(entityName) && UtilValidate
                    .isNotEmpty(displayFields)) || "Y".equals(noConditionFind)) {
            ...
            EntityConditionList<EntityCondition> entityConditionList = EntityCondition
                    .makeCondition(mainAndConds, EntityOperator.AND);
            
            if (UtilValidate.isEmpty(viewSize)) {
                viewSize = new Integer(UtilProperties.getPropertyValue(
                        "widget", "widget.autocompleter.defaultViewSize"));
            }
            
            Integer autocompleterViewSize = UtilValidate.isNotEmpty(viewSize) ? viewSize
                    : 10;
            ...
            try {
                List<GenericValue> autocompleteOptions = delegator.findList(
                        entityName, entityConditionList, displayFieldSet,
                        StringUtil.toList(displayFields), findOptions, false);
                request.setAttribute("options", autocompleteOptions);
                return ServiceUtil.returnSuccess();
            } catch (GenericEntityException e) {
                return ServiceUtil
                        .returnError("Could not find options because: "
                                + e.getMessage());
            }
        } else {
            return ServiceUtil
                    .returnError("Could not find entity name or display fields.");
        }
    }
}
